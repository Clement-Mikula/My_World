...
main(int argc, char *argv[])
{
    ...
    for(y=0; y<4000; y++)
        for(x=0; x<4000; x++) {
            random = perlin2d(x, y, 0.1, 4);
            printf("random = %f    x = %d, y = %d\n", random, x, y);
        }
...
void move_vertex_perlin(game_t *game)
{
    for (int i = 0; i < MAP_X; i++)
    {
        ...
        for (int j = 0; j < MAP_Y; j++)
        {
            ...
            //printf("tu clique\n");
            //random = (float)(rand() % 5);
            game->int_map[i][j] = perlin2d(i, j, 0.1, 4) * 750; //4
            game->int_map[i][j] -= 500;
            //printf("random = %f    x = %d, y = %d\n", game->map[i][j].x, i, j);

            //game->map[i][j].x = x;
            //game->map[i][j].y = y;
            //perlin2d(game->map[i][j].x, game->map[i][j].y, 0.1, 4);
            //game->int_map[i][j] += 10;
            //}
        }
    }
